  ...
  return (
    <nav className="relative z-50 flex justify-between items-center px-4 sm:px-8 md:px-16 py-6 md:py-8">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <Link 
          href="/login" 
          onClick={() => window.logQuestEvent?.('conversion_intent', { button: 'Nav Login' })}
          className="px-3 sm:px-8 py-3 text-sky-600 font-black hover:bg-sky-50 rounded-xl md:rounded-2xl transition-all text-xs sm:text-base"
        >
          Log In
        </Link>
        <Link 
          href="/signup" 
          onClick={() => window.logQuestEvent?.('conversion_intent', { button: 'Nav Signup' })}
          className="px-4 sm:px-8 py-3 sm:py-4 bg-sky-500 text-white font-black rounded-xl md:rounded-2xl shadow-[0_4px_0_rgb(7,118,181)] sm:shadow-[0_8px_0_rgb(7,118,181)] hover:translate-y-[2px] active:shadow-none active:translate-y-[4px] transition-all flex items-center gap-1 sm:gap-2 group text-xs sm:text-base whitespace-nowrap"
        >
